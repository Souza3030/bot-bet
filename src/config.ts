...
export const config = {
  ...
  queue: {
    mode: "solo" as const,
    // Tempo máximo (ms) que uma aposta paga pode esperar por um adversário do mesmo valor antes de ser removida/reembolsada
    betQueueTimeoutMs: 20 * 60 * 1000,
    // Intervalo (ms) de varredura para remover apostas inativas da fila
    betQueueTimeoutCheckIntervalMs: 60 * 1000,
    // Tempo (ms) que os 2 jogadores pareados têm para confirmar presença (check-in)
  ...
  bet: {
    // Valores fixos (em reais) disponíveis no select de aposta.
